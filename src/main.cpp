...
#if defined(NDEBUG)
# error "BitcoinLove cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xce65c507fe3980e7f62a14f2982d877b44fce9767e7d863790838e4419101468");
...
const string strMessageMagic = "BitcoinLove Signed Message:\n";
...
{
    // BitcoinLove: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitcoinLove: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitcoinLove: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xce65c507fe3980e7f62a14f2982d877b44fce9767e7d863790838e4419101468");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is BitcoinLove";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1518691220;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1552227;
        ...
        assert(block.hashMerkleRoot == uint256("0x88aaee6193db811c2154fae7164e2ea4b81d13ca30024b2a501c280599234f4b"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // BitcoinLove: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinLoveMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinLoveMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoinLoveMiner(CWallet *pwallet)
{
    printf("BitcoinLoveMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BitcoinLove-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoinLoveMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoinLoveMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinLoveMiner, pwallet));
}
